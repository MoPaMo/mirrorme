      ...
      if (message.content === "!m-start") {
        ...
        db.get(message.guild.id).then((val) => {
          if (val) {
            ...
            message.channel.send(
              "Looks like we didn't activly mirrored this server 🤷. If you want to fo that, type `!m-start`"
            );
          }
        });
...
<https://discord.com/oauth2/authorize?client_id=835079528770043925&scope=bot&permissions=335694913&response_type=code&redirect_uri=https%3A%2F%2Fmirror.mopamo.repl.co%2Ffinish>`
            ...
            if (role) {
              role.delete(
                "Since mirrorme left, there's no use for this role. Invite again: https://discord.com/oauth2/authorize?client_id=835079528770043925&scope=bot&permissions=335694913&response_type=code&redirect_uri=https%3A%2F%2Fmirror.mopamo.repl.co%2Ffinish"
              );
            }
            console.log(role);
            message.react("🍃");
